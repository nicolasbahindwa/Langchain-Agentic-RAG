# """
# Enhanced document processing module with robust error handling and file permission management
# """
# import os
# import hashlib
# import logging
...
#         # File type handling with more extensions
#         self.supported_extensions = supported_extensions or [
#             '.txt', '.pdf', '.csv', '.docx', '.doc', '.json', 
#             '.pptx', '.html', '.md', '.rtf', '.odt'
#         ]
        
#         # Document loader registry with fallback options
#         self.loaders = {
...
#         Returns:
#             True if file can be safely accessed, False otherwise
#         """
#         try:
...
#         Returns:
#             Hash string (content-based if possible, metadata-based otherwise)
#         """
#         path = Path(file_path)
        
#         # Strategy 1: Content-based hash (preferred)
#         try:
...
#         # Strategy 2: Metadata-based hash (fallback)
#         try:
#             stat = path.stat()
...
#         # Strategy 3: Path-based hash (last resort)
#         try:
...
#             # Filter out empty pages and clean content
#             valid_docs = []
#             for doc in documents:
#                 content = doc.page_content.strip()
#                 if content and len(content) > 10:  # Skip very short pages
...
#         try:
#             # Get appropriate loader function
...
#             # Apply metadata to all document parts
#             for doc in documents:
...
#         """
#         Split documents into chunks with enhanced error handling
...
#                 # Ensure we have a file hash for chunk ID
...
#             logger.info(f"ℹ️ Split {len(valid_docs)} docs into {len(processed_chunks)} chunks")
...
#         Returns:
#             List of processed document chunks ready for vectorization
#         """
...
#                 'can_access': self._safe_file_access_check(path)
#             }
#         except Exception as e:
#             logger.failure(f"❌ Failed to get metadata for {path}: {e}")
#             return {'error': str(e)}
    
#     def process_directory(self, dir_path: Union[str, Path], recursive: bool = True) -> List[Document]:
#         """
#         Process all supported files in a directory with error handling
...
#         logger.info(f"✅ Directory processing complete: {processed_files} files processed, "
