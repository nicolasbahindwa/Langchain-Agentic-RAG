...
# from Agents.prompts import research_agent_prompt_with_mcp, compress_research_system_prompt, compress_research_human_message
# from Agents.state_research import ResearcherState, ResearcherOutputState
# from Agents.research_utils import get_today_str, think_tool, get_current_dir
...
async def get_mcp_tools():
    """Get MCP tools, initializing client in a separate thread if needed."""
    global _client, _mcp_tools_cache
    ...
    if _client is None:
        # Initialize MCP client in a separate thread to avoid blocking calls
        _client = await asyncio.to_thread(lambda: MultiServerMCPClient(mcp_config))
    
    # Get tools (this is async and should not block)
    try:
        _mcp_tools_cache = await _client.get_tools()
        return _mcp_tools_cache
    except Exception as e:
        print(f"Warning: Failed to initialize MCP tools: {e}")
        # Return empty list if MCP initialization fails
        return []
...
async def llm_call(state: ResearcherState):
    ...
    This node:
    1. Retrieves available tools from MCP server (in a separate thread)
    2. Binds tools to the language model
    3. Processes user input and decides on tool usage

    Returns updated state with model response.
    """
    # Get available tools from MCP server (non-blocking)
        ...
        for tool_call in tool_calls:
            ...
            if tool is None:
                ...
                try:
                    ...
                    observation = f"Error executing {tool_call['name']}: {str(e)}"
            observations.append(observation)
