...
class SearchProvider(ABC):
    """Abstract base class for search providers - returns raw results"""
    
    @abstractmethod
    def search(self, query: str, **kwargs) -> Any:
        """Perform search and return raw results as-is from the provider"""
        pass


class TavilySearchProvider(SearchProvider):
    """Tavily search provider - returns raw results"""
    ...
    def search(self, query: str, **kwargs) -> Any:
        """Search using Tavily - returns raw results exactly as received"""
        try:
            # Try different methods to invoke Tavily
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if hasattr(self.tool, 'search'):
                            return self.tool.search(query, **kwargs)
                        else:
                            raise Exception("No working Tavily method found")
        except Exception as e:
            return {"error": f"Tavily API error: {str(e)}"}


class WikipediaSearchProvider(SearchProvider):
    """Wikipedia search provider - returns raw results"""
    ...
    def search(self, query: str, max_results: int = 5, auto_suggest: bool = True, 
               full_content: bool = False, summary_sentences: int = 3, **kwargs) -> Any:
        """Search Wikipedia - returns raw results"""
        try:
            # Search for articles
            search_titles = wikipedia.search(query, results=max_results, suggestion=auto_suggest)
            
            if not search_titles:
                return {"message": "No Wikipedia articles found", "results": []}
            ...
            for title in search_titles:
                ...
                try:
                    ...
                    result = {
                        "title": title,
                        "disambiguation": True,
                        "options": e.options[:10],  # Limit options
...
class DuckDuckGoSearchProvider(SearchProvider):
    """DuckDuckGo search provider - returns raw results"""
    
    def __init__(self):
        self.ddgs = DDGS()
    
    def search(self, query: str, max_results: int = 5, region: str = "wt-wt", 
               safesearch: str = "moderate", timelimit: Optional[str] = None, **kwargs) -> Any:
        """Search using DuckDuckGo - returns raw results"""
        try:
            results = []
...
class SearchManager:
    """Raw search manager - returns unformatted results from providers"""
    ...
    def search(self, query: str, provider: str = "duckduckgo", **kwargs) -> Any:
        """
        Perform search using specified provider - returns raw results
        ...
        return self.providers[provider].search(query, **kwargs)
    
    def multi_search(self, query: str, providers: List[str] = None, **kwargs) -> Dict[str, Any]:
        """
        Search across multiple providers - returns raw results from each
        ...
        for provider in providers:
            if provider in self.providers:
                results[provider] = self.search(query, provider, **kwargs)
...
def create_search_manager(wikipedia_language: str = "en") -> SearchManager:
    ...
    if TavilySearchResults is not None:
        try:
            if hasattr(config, 'api_keys') and hasattr(config.api_keys, 'tavily_api_key') and config.api_keys.tavily_api_key:
                manager.add_provider("tavily", TavilySearchProvider())
                print("Tavily provider initialized")
            else:
...
if __name__ == "__main__":
    ...
    for provider in search_manager.get_available_providers():
        print(f"\n=== {provider.upper()} ===")
        result = search_manager.search(query, provider=provider, max_results=2)
        print(json.dumps(result, indent=2, default=str))
    
    # Test multi-search
    print(f"\n=== MULTI SEARCH ===")
    multi_result = search_manager.multi_search(query, max_results=2)
